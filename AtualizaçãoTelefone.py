...
def abrir_gecobi_com_cpj():
    ...
    c = wait.until(EC.element_to_be_clickable((By.XPATH, "//i[contains(@class,'material-icons') and normalize-space()='calculate']")))
    time.sleep(0.5)
    c.click()

    o = wait.until(EC.element_to_be_clickable((By.XPATH, "//div[contains(@class,'q-item__label') and normalize-space()='Operação']")))
    time.sleep(0.5)
    o.click()

    t = wait.until(EC.element_to_be_clickable((By.XPATH, "//div[contains(@class,'q-item__section') and contains(@class,'q-item__section--main') and normalize-space()='Telefones']")))
    time.sleep(0.5)
    t.click()
...
for class_name, select_value in status_config:
    ...
    driver.switch_to.default_content()
    WebDriverWait(driver, 10).until(EC.frame_to_be_available_and_switch_to_it((By.CSS_SELECTOR, "iframe.frame-legado")))
    ...
    if not textarea:
        textarea = find_element_any_frame(driver, By.XPATH, "/html/body/table[2]/tbody/tr[6]/td[2]/textarea", max_depth=6)
